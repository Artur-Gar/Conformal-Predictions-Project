...
class XRaysTrainDataset(Dataset):
    ...
    def get_train_val_df(self):
        ...
        train_val_list = set(self.get_train_val_list())

        print('\nbuilding train_val_df...')
        # self.df['image_links'] holds full paths; we compare their basenames to the list
        mask = self.df['image_links'].apply(lambda p: os.path.basename(p)).isin(train_val_list)
        train_val_df = self.df[mask].copy()
    ...
    def get_train_val_list(self):
        list_path = os.path.join(self.data_dir, 'train_val_list.txt')
        with open(list_path, 'r') as f:
            train_val_list = f.read().strip().split('\n')
...
class XRaysTestDataset(Dataset):
    ...
    def get_test_df(self):
        ...
        test_list = set(self.get_test_list())

        print('\nbuilding test_df...')
        mask = self.df['image_links'].apply(lambda p: os.path.basename(p)).isin(test_list)
    ...
    def get_test_list(self):
        list_path = os.path.join(self.data_dir, 'test_list.txt')
        with open(list_path, 'r') as f:
